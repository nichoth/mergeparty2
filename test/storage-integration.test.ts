...
test('Storage integration - persistence across connections', async t => {
    ...
    try {
        ...
        if (doc) {
            ...
            t.equal((doc as any)?.text, 'Hello from first connection',
                'Document content should persist')
            t.equal((doc as any)?.counter, 1,
                'Document properties should persist')
        } else {
            return
        }
...
test('Storage integration - debug endpoint', async t => {
    ...
    try {
        ...
        const serverReady = await waitForServer(PARTYKIT_HOST, 10)
        if (!serverReady) {
            console.log('PartyKit failed to start - skipping integration test')
            return
        }
        ...
        t.ok(typeof data === 'object',
            'Debug endpoint should return JSON object')
    } catch (error) {
...
function startPartykitServer ():Promise<{
    ...
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            resolve({
                process: serverProcess as ServerProcess,
                ready: readyPromise
            })
        }, 1000)
    })
